...
async function generateAndSendReport(managerEmail, specificScanData = null) {
  ...
  try {
    // 1. Fetch overall database stats from Postgres
    const compQuery = await pool.query("SELECT COUNT(*) AS count FROM competitors");
    const gamesQuery = await pool.query("SELECT COUNT(*) AS count FROM games");
    
    const stats = {
      competitors: compQuery.rows[0].count,
      games: gamesQuery.rows[0].count,
    };
    ...
    // 3. Build a native HTML template
    const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
